...
void mostrarCidade(struct Cidade cidade, int cartaCodigo) {
    ...
    printf("Nome: %s\n", cidade.cidade_nome);
    printf("Populacao: %ld\n", cidade.populacao);
    printf("Area (em quilometro quadrado): %.2lf\n", cidade.area);  // Corrigido para mostrar a área com 2 casas decimais. (.2lf)
    printf("PIB: R$%.2lf bi\n", cidade.pib);                            // Mostrar o PIB com 2 casas decimais. (.2lf)
...
int DeterminarVencedor(struct Cidade cidade1, struct Cidade cidade2, int *placar1, int *placar2) {
    ...
    *placar2 = 0;   // Estrutura o placar das cartas, que começa em 0.
    ...
    } else if (*placar2 > *placar1) {                // Se o placar final da Carta 2 for maior que o da Carta 1,
        return 2;                                  // ...A Carta 2 vence.

    } else {
        return 0;                     // Se não 'retornou 1' e nem '2',  significa que ocorreu um empate entre as duas cartas (0).
    }
}
...
int main() {
    ...
    do {
        setbuf(stdin, NULL);       // Garantir que o buffer de entrada esteja limpo para evitar leituras incorretas.
        ...
        // Verifica se a resposta do usuário é uma das opções válidas para continuar o jogo.
             ...
             strcmp(resposta, "yes") == 0 || strcmp(resposta, "'yes'") == 0 || strcmp(resposta, "'yes'.") == 0);

    exit(1); // Fecha o programa caso a resposta não seja uma das opções válidas acima para continuar.
    return 0;
}
